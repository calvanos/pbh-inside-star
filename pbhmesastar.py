...
#def calculate_sound_speed(T, mu, cst):
    # Calculate the speed of sound in the stellar material
   # return np.sqrt(cst.gamma * cst.k_B * T / (mu * cst.m_H))
    
#def m_dot_pbh(r,rho,cst,T,mu):
    #c_s= calculate_sound_speed(T,mu,cst)
   # return (16*np.pi*0.08)/(1-0.08)*0.1/(5/3*cst.c_s)*rho*(cst.G*M_pbh/cst.c)**2

#def L_bh(kappa,m_dot_pbh,cst):
 #   Ledd=(4*np.pi*cst.c*cst.G*M_pbh)/kappa
  #  Lbondi= 0.08/(1-0.08)*m_dot_pbh*(cst.c)**2
   # return np.minimum(Ledd,Lbondi)

#def m_dot(kappa,m_dot_pbh,cst,L_bh):
   # Ledd=(4*np.pi*cst.c*cst.G*M_pbh)/kappa
   # Lbondi= 0.08/(1-0.08)*(16*np.pi*0.08)/(1-0.08)*0.1/(5/3*cst.c_s)*rho*(cst.G*M_pbh/cst.c)**2*(cst.c)**2
   # L_bh=np.minimum(Ledd,Lbondi)
   # return 12.5*L_bh/(cst.c)**2 

def dPdr(r, M_r, rho,cst):
      return -cst.G*rho*(M_r+M_pbh)/r**2

def dMdr(r, rho, cst):
      #c_s=(5*P/rho)**(1/2)
      c_s=1.6*3.5*10**7
      Rbondi=2*cst.G*M_pbh/(c_s)**2
      if(r <= Rbondi):
          return ((4*np.pi*rho*Rbondi**3*r**(1/2)))
      else:
          return ((4.0e0*np.pi*rho*r**2))

#(8*np.pi/3*Rbondi**3*r**(-3/2)) + M_pbh 
#((4.0e0*np.pi*rho*r**2)-(16*np.pi*0.08)/(1-0.08)*0.1/(5/3*c_s)*rho*(cst.G*M_pbh/cst.c)**2)
#(16*np.pi*0.08)/(1-0.08)*0.1/(5/3*cst.c_s)*rho*(cst.G*M_pbh/cst.c)**2      
#0.08/(1-0.08)*(16*np.pi*0.08)/(1-0.08)*0.1/(5/3*cst.c_s)*rho*(cst.G*M_pbh/cst.c)**2*(cst.c)**2

def dLdr(r, rho, epslon,kappa,cst):
      #c_s=(5*P/rho)**(1/2)
      c_s=1.6*3.5*10**7
      k=0.3+1.2*(1)**(-1/3)
      Rbondi=2*cst.G*M_pbh/(c_s)**2
      Ledd=(4*np.pi*cst.c*cst.G*M_pbh)/k
      Lbondi= 16*np.pi*0.1*rho*3*(cst.G*M_pbh)**2/(5*c_s)
      L_bh=np.minimum(Ledd,Lbondi)
      if(r<=2*Rbondi):
          return (4.0e0*np.pi*rho*epslon*r**2 + L_bh)
      else:
...
def FUNDEQ(r,f,irc,X,Z,XCNO,mu,izone,cst):
      ...
      dfdr[2] = dLdr(r, rho, epslon, kappa, cst)
...
#
      ...
      for i in range(Nsrtp1,Nstop):
          ...
          dfdr[2]  = dLdr(r[im1], rho[im1], epslon[im1], kappa[im1] , cst)
